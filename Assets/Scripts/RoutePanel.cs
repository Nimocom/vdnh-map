...
public class RoutePanel : MonoBehaviour, IPointerDownHandler
{
    ...
    public void InitializePanel(Route route)
    {
        ...
        {
            var building = BuildingsManager.Instance.Buildings.SingleOrDefault(x => x.PlaceData.properties.id == route.Points[i]);
        ...
        {
            totalDistance += Vector3.Distance(buildings[i].point.position, buildings[i + 1].point.position);
        }
    ...
    public void OnPointerDown(PointerEventData eventData)
    {
        ...
        CameraTarget.Instance.LookAt(buildings[0].point.position);
