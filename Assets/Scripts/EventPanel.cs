...
public class EventPanel : MonoBehaviour,IPointerDownHandler
{
    ...
    public void InitializePanel(EventData eventData)
    {
        ...
        building = BuildingsManager.Instance.Buildings.SingleOrDefault(x => x.PlaceData.properties.id == data.LocationID);

        if (building != null)
            location.SetText(building.PlaceData.properties.title);
    }
